...
export default function TeamPage() {
  ...
  const fetchTeamData = async (forceRefresh = false) => {
    ...
    try {
      ...
      // First fetch team details from the specific team endpoint
      const teamResponse = await axios.get(`${API_URL}/teams/${teamId}`);
      
      if (!teamResponse.data || !teamResponse.data.team_info) {
      ...
      // Set team data from response
      setTeam({
        id: teamResponse.data.team_info.id,
        full_name: teamResponse.data.team_info.name,
        city: teamResponse.data.team_info.city,
        abbreviation: teamResponse.data.team_info.abbreviation,
        conference: teamResponse.data.team_info.conference,
        division: teamResponse.data.team_info.division,
        state: teamResponse.data.team_info.state || '',
        year_founded: teamResponse.data.team_info.year_founded || ''
      });
      
      // Set roster data from the same response
      if (teamResponse.data.roster && Array.isArray(teamResponse.data.roster)) {
        setPlayers(teamResponse.data.roster);
      } else {
        // Fallback to separate player endpoint if roster is empty
        try {
          const playersResponse = await axios.get(`${API_URL}/players/team/${teamId}`);
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {sortedPlayers.map((player) => (
                  <Link href={`/player/${player.id || player.player_id}`} key={player.id || player.player_id} className="no-underline">
                    <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl shadow-lg overflow-hidden transform transition-all hover:scale-105">
                      <div className="relative">
                        <div className="aspect-w-1 aspect-h-1 w-full overflow-hidden bg-gray-200 dark:bg-gray-700">
                          <Image
                            src={`https://cdn.nba.com/headshots/nba/latest/1040x760/${player.id || player.player_id}.png`}
                            alt={player.name}
                            width={300}
                            height={300}
